...
\begin{frame}{GUARDIAN and the Curse of Rarity}{The wider research programme}
  \begin{columns}[T,onlytextwidth]
    ...
    \begin{column}{0.46\textwidth}
      \begin{block}{Why a \emph{synthesised} world?}
        \small
        You cannot collect more Eaton Fires. The only way to enlarge the
        tail of the distribution is to \gold{generate} it --- and the only
        way that generation is defensible is if every mutation stays
        physically admissible.\\[4pt]
        Hence the two halves of GUARDIAN: a \textbf{mutation operator} that
        proposes extreme worlds, and an \textbf{admissibility filter} that
        throws away the ones physics forbids.
      \end{block}
      \vskip2pt
...
\begin{frame}{Geospatial Layer}{The shared reference frame}
  \begin{columns}[T,onlytextwidth]
    \begin{column}{0.55\textwidth}
      \begin{itemize}
        \item \code{SOCAL\_BOUNDS} $=(-121.3,\,32.4,\,-113.7,\,37.7)$,
              EPSG:4326. Row 0 is the \emph{north} edge.
        \item Three raster sources, in order of fidelity:
          \begin{itemize}
            \item \code{from\_rasters()} --- LANDFIRE FBFM40 fuel + 3DEP
                  elevation (needs \code{rasterio}).
            \item \code{socal\_from\_srtm()} --- SRTM GL3 ($\approx$90\,m),
                  4$\times$3 tile mosaic $\rightarrow$ $600\times760$ grid.
            \item \code{synthetic\_socal()} --- deterministic CI fallback.
          \end{itemize}
        \item \gold{Real terrain is the default} --- every experiment YAML
              but the smoke test sets \code{use\_real\_dem: true}. The 71\,MB
              mosaic is git-ignored: fetch it once with
              \code{fetch\_dem\_tiles.py}.
        \item Without it a run \emph{silently} uses synthetic terrain.
      \end{itemize}
      \begin{alertblock}{Scale trap}
        \small One statewide cell $\approx$\,947\,m, so retardant drops are
        sub-cell --- hence fine grids ($\approx$50\,m) for suppression work.
      \end{alertblock}
    \end{column}
    \begin{column}{0.43\textwidth}
      \centering
      \includegraphics[width=0.92\linewidth]{fire_growth.png}\\[2pt]
  ...
  \begin{columns}[T,onlytextwidth]
    ...
    \begin{column}{0.48\textwidth}
      ...
      \begin{alertblock}{State this honestly}
        ...
        \vskip3pt
        Real-DEM long-runs \emph{are} committed --- the two
        \code{\ldots\_drl\_long.yml} experiments
        (\code{episodes: 400}, \code{use\_real\_dem: true}). They have
        simply not been run yet.
        \gold{An open task, not a missing feature.}
      \end{alertblock}
    \end{column}
  \end{columns}
\end{frame}
